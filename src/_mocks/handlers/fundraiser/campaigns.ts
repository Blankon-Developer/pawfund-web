...
export function generateCampaigns(count: number): FundariserCampaignItemData[] {
  return Array.from({ length: count }, (_, index) => {
    const goalAmount = 10_000_000 + index * 5_000_000
    const progress = index == 3 ? 1 : Math.random()
    const raisedAmount = Math.floor(goalAmount * progress)

    const status: FundraiserCampaignStatus =
      progress >= 1 ? "completed" : index + 1 == count ? "cancelled" : "active"
    ...
    return {
      ...
      imageUrl: `https://picsum.photos/seed/campaign-${index}/800/600`,
      endAt: new Date(
...
const ALL_CAMPAIGNS = generateCampaigns(10)

export const getMyCampaigns = http.get(
  buildApiUrl("/fundraiser/campaigns"),
  async ({ request }) => {
    ...
    return HttpResponse.json<ApiSuccessType<FundariserCampaignItemData[]>>(
      {
        code: "SUCCESS",
